...
function makeSearchSummary(searchData, searchType) {
  ...
  const contextLine = searchType === "image"
    ? `I searched images for "${queryText}". The most relevant visual lead I found is "${contextTitle}". These are image leads, so use the links at the end to inspect the original source.`
    : `I searched the web for "${queryText}". The strongest result I found is "${contextTitle}"${contextSnippet ? `, and it says ${contextSnippet}` : ""}. In short, that is the best quick context I can give from the search results.`;
